...
*Using anyone of programming languages, with only two operations: string concatenation and type conversion, there is no need to use template languages, such as format in "printf" of C language, [[https://mustache.github.io/][mustache]], [[https://shopify.github.io/liquid/][liquid]], [[http://jinja.pocoo.org/][jinja2]] and etc.*
...
#+BEGIN_SRC elisp
  ...
  (message
     (concat "Hello, world, my name is "
             ...
             (int-to-string MyAge)))
#+END_SRC
"message" output the concatenated string into console.

"concat" concatenate the strings.

"int-to-string" converse the "int" type variable into string.

all is direct. we don't need to parse "Hello world, my name is %s, my age is %d".
